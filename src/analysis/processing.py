...
def trigger_selection(selection, events, cfg):
    ...
    # Check if any event attributes are missing
    pass_all = np.zeros(events.size) == 0
    pass_none = ~pass_all
    ...
    if cfg.RUN.SYNC: # Synchronization mode
        ...
        if events['is_data']:
            selection.add('filt_met', mask_and(events, cfg.FILTERS.DATA))
        else:
            selection.add('filt_met', mask_and(events, cfg.FILTERS.MC))
        selection.add('trig_met', mask_or(events, cfg.TRIGGERS.MET))

        # Electron trigger overlap
        if events['is_data']:
            if "SinglePhoton" in dataset:
                # Backup photon trigger, but not main electron trigger
                trig_ele = mask_or(events, cfg.TRIGGERS.ELECTRON.SINGLE_BACKUP) & (~mask_or(events, cfg.TRIGGERS.ELECTRON.SINGLE))
            elif "SingleElectron" in dataset:
                # Main electron trigger, no check for backup
                trig_ele = mask_or(events, cfg.TRIGGERS.ELECTRON.SINGLE)
        ...
        # Photon trigger:
        if (not events['is_data']) or ('SinglePhoton' in dataset) or ('EGamma' in dataset):
            trig_photon = mask_or(events, cfg.TRIGGERS.PHOTON.SINGLE)
        else:
            trig_photon = pass_none
        selection.add('trig_photon', trig_photon)

        for trgname in cfg.TRIGGERS.HT.GAMMAEFF:
            if (not events['is_data']) or ('JetHT' in dataset):
                selection.add(trgname, mask_or(events,[trgname]))
            else:
                selection.add(trgname, np.ones(events.size)==1)

        # Muon trigger
        selection.add('trig_mu', mask_or(events, cfg.TRIGGERS.MUON.SINGLE))

    return selection


def define_weight_counters(output, events, weights, rand_datasets):
    """Add weight counters to coffea processor output.
    ...
    :type events:
    :param weights:
    :type weights: DynaConf object
    """

    dataset = events['dataset']
    output['nevents'][dataset] += events.size
    if not events['is_data']:
        if len(rand_datasets):
            ...
            for ds, short in rand_datasets.items():
                dsmask = events[f'GenModel_{ds}']
                output['nevents'][short] += dsmask.sum()
                # Split per sub-dataset
                output['sumw'][short] +=  getattr(events, f'genEventSumw_{ds}', 0)
                output['sumw2'][short] +=  getattr(events,f'genEventSumw2_{ds}', 0)
                output['sumw_pileup'][short] +=  weights.partial_weight(include=['pileup'])[dsmask].sum()

                # Integrated for the whole dataset
                output['sumw'][dataset] +=  getattr(events, f'genEventSumw_{ds}', 0)
                output['sumw2'][dataset] +=  getattr(events,f'genEventSumw2_{ds}', 0)
                output['sumw_pileup'][dataset] +=  weights.partial_weight(include=['pileup'])[dsmask].sum()
        else:
            # For normal datasets, no splitting is necessary
            output['sumw'][dataset] +=  events[f'genEventSumw']
            output['sumw2'][dataset] +=  events[f'genEventSumw2']
...
class hhbbtautauProcessor(processor.ProcessorABC):
    ...
    def process(self, events):
        ...
        if not events['is_data']:
            ...
            events['leadak8_gen_match_ak8_eta'] = best_pair.i1.eta
            events['leadak8_gen_match_ak8_phi'] = best_pair.i1.phi
            events['leadak8_gen_match_ak8_mass'] = best_pair.i1.mass

        events['leadak8_pt']       = ak8.pt[leadak8_index]
        events['leadak8_eta']      = ak8.eta[leadak8_index]
        events['leadak8_abseta']   = ak8.abseta[leadak8_index]
        events['leadak8_tau21']    = ak8.tau2[leadak8_index] / ak8.tau1[leadak8_index]
        events['leadak8_mass']     = ak8.mass[leadak8_index]
        events['leadak8_wvsqcdmd'] = ak8.wvsqcdmd[leadak8_index]
        events['leadak8_wvsqcd']   = ak8.wvsqcd[leadak8_index]

        leadak8_pt_eta = (events['leadak8_pt'].max() > cfg.SELECTION.SIGNAL.leadak8.PT) \
                         & (events['leadak8_abseta'] < cfg.SELECTION.SIGNAL.leadak8.ETA).any()

        selection.add('leadak8_pt_eta', leadak8_pt_eta)
        selection.add('leadak8_id',(ak8.tightId[leadak8_index]).any())

        # Mono-V selection
        selection.add('leadak8_tau21', (events['leadak8_tau21'] < cfg.SELECTION.SIGNAL.LEADAK8.TAU21).any())

        selection.add('leadak8_mass', ((events['leadak8_mass'] > cfg.SELECTION.SIGNAL.LEADAK8.MASS.MIN) \
                                    & (events['leadak8_mass'] < cfg.SELECTION.SIGNAL.LEADAK8.MASS.MAX)).any())
        ...
        selection.add('leadak8_wvsqcd_loosemd', ((events['leadak8_wvsqcdmd'] > cfg.WTAG.LOOSEMD)
                                               & (events['leadak8_wvsqcdmd'] < cfg.WTAG.TIGHTMD)).any())
        selection.add('leadak8_wvsqcd_tightmd', ((events['leadak8_wvsqcdmd'] > cfg.WTAG.TIGHTMD)).any())
        selection.add('leadak8_wvsqcd_loose', ((events['leadak8_wvsqcd'] > cfg.WTAG.LOOSE)
                                             & (events['leadak8_wvsqcd'] < cfg.WTAG.TIGHT)).any())
        selection.add('leadak8_wvsqcd_medium', ((events['leadak8_wvsqcd'] > cfg.WTAG.MEDIUM)).any())
        selection.add('leadak8_wvsqcd_tight', ((events['leadak8_wvsqcd'] > cfg.WTAG.TIGHT)).any())
        ...
        selection.add('at_least_one_tight_mu', events['is_tight_muon'].any())
        ...
        selection.add('mt_mu', events['MT_mu'] < cfg.SELECTION.CONTROL.SINGLEMU.MT)
        ...
        selection.add('at_least_one_tight_el', events['is_tight_electron'].any())
        ...
        selection.add('mt_el', events['MT_el'] < cfg.SELECTION.CONTROL.SINGLEEL.MT)

        # Photon CR
        leadphoton_index=photons.pt.argmax()

        events['is_tight_photon'] = photons.mediumId & photons.barrel

        selection.add('one_photon', photons.counts==1)
        selection.add('at_least_one_tight_photon', events['is_tight_photon'].any())
        ...
        if gen_v_pt is not None:
            output['genvpt_check'].fill(vpt=gen_v_pt,type="Nano", dataset=dataset, weight=events['Generator_weight'])

        if 'LHE_HT' in events:
            output['lhe_ht'].fill(dataset=dataset, ht=events['LHE_HT'])

        # Weights
        evaluator = evaluator_from_config(cfg)

        weights = processor.Weights(size=events.size, storeIndividual=True)
        if not events['is_data']:
            weights.add('gen', events['Generator_weight'])

            try:
                weights.add('prefire', events['PrefireWeight'])
            except KeyError:
                weights.add('prefire', np.ones(events.size))

            weights = candidate_weights(weights, events, evaluator, muons, electrons, photons, cfg)

            # B jet veto weights
            bsf_variations = btag_weights(bjets,cfg)
            weights.add("bveto", (1-bsf_variations["central"]).prod())

            weights = pileup_weights(weights, events, evaluator, cfg)
            if not (gen_v_pt is None):
                theory_weights = theory_weights_monojet(
                                                         ...
                                                         evaluator=evaluator,
                                                         gen_v_pt=gen_v_pt,
                                                         gen_ak8_mass=events['leadak8_gen_match_ak8_mass'].max()
                                                         )


            # Diboson NLO
            diboson_nlo_weights(events, evaluator, gen)
            weights.add('weight_diboson_nlo', events['weight_diboson_nlo'])

        # Randomized Parameter data sets
        # keep track of the mapping
        rand_datasets = rand_dataset_dict(events.keys(), events['year'])

        # Sum of all weights to use for normalization
        define_weight_counters(output, events, weights, rand_datasets)

        regions = monojet_regions(cfg)

        # Get veto weights (only for MC)
        if not events['is_data']:
            veto_weights = get_veto_weights(events, cfg, evaluator, electrons, muons, taus, do_variations=True)
        ...
        for region, cuts in regions.items():
            ...
            if re.match('sr_.*', region):
                ...
                recoil_pt = events['recoil_pt']
                recoil_phi = events['recoil_phi']
            ...
            if not events['is_data']:
                ...
                if re.match(r'cr_(\d+)e.*', region):
                    ...
                    photon_trigger_sf(region_weights, photons, events)
                ...
                if not (gen_v_pt is None):
                    ...
                    central = bool(re.match(r'cr_1(e|m)_.*', region)) and events['is_lo_w']
                    central |= bool(re.match(r'cr_2(e|m)_.*', region)) and events['is_lo_z']
            ...
            if not (events['is_data']):
                ...
                for wp in ['loose','tight','medium']:
                    if re.match(f'.*_{wp}_v.*', region):
                        ...
                        elif events['is_lo_w'] or events['is_nlo_w']:
                        ...
                        elif events['is_lo_g'] or events['is_nlo_g'] or re.match(r'QCD.*', dataset):
            ...
            # Blinding
            if(self._blind and events['is_data'] and region.startswith('sr')):
                continue

            # Cutflow plot for signal and control regions
            if any(x in region for x in ["sr", "cr", "tr"]):
                output['cutflow_' + region][dataset]['all']+=events.size
            ...
            if cfg.RUN.SAVE.TREE:
                if re.match(cfg.RUN.SAVE.TREEREGIONS, region):
                    # General properties
                    output['tree_int64'][region]["event"]                   += processor.column_accumulator(events["event"][mask])
                    output['tree_int64'][region]["run"]                     += processor.column_accumulator(events["run"][mask])
                    output['tree_int64'][region]["lumi"]                    += processor.column_accumulator(events["luminosityBlock"][mask])
                    ...
                    if region=='inclusive':
                        ...
                        output['tree_float16'][region]["met_pt_nojer"]          += processor.column_accumulator(events['MET_pt_nom' if events['year']==2018 else 'METFixEE2017_pt_nom'][mask])
                        output['tree_float16'][region]["met_phi_nojer"]         += processor.column_accumulator(events['MET_phi_nom' if events['year']==2018 else 'METFixEE2017_phi_nom'][mask])
                        ...
                        output['tree_float16'][region]["mindphijr"]            += processor.column_accumulator(events['minDPhiJetRecoil'][mask])

                        output['tree_float16'][region]["calomet_pt"]            += processor.column_accumulator(events['CaloMET_pt'][mask])


                        # MC quantities
                        if not events['is_data']:
                            if gen_v_pt is not None:
                                output['tree_float16'][region]["gen_v_pt"]              += processor.column_accumulator(gen_v_pt[mask] if gen_v_pt is not None else np.zeros(len(events["event"][mask])))
                        ...
                        if re.match('.*_1m_.*', region):
                            output['tree_float16'][region]["mt"]  += processor.column_accumulator(events['MT_mu'][mask])

                        # Transverse mass single electron
                        if re.match('.*_1e_.*', region):
                            output['tree_float16'][region]["mt"]  += processor.column_accumulator(events['MT_el'][mask])
                        ...
                        if re.match('.*_v', region):
                            for key in ['leadak8_pt','leadak8_eta','leadak8_tau21','leadak8_mass','leadak8_wvsqcd']:
                                output['tree_float16'][region][key]  += processor.column_accumulator(events[key][mask].max())

                            if not events['is_data']:
                                for key in ['leadak8_gen_match_v','leadak8_gen_match_ak8']:
                                    output['tree_float16'][region][key]  += processor.column_accumulator(events[key][mask])
            ...
            if cfg.RUN.SAVE.PASSING:
                ...
                save_mask = mask & ((events['event']%cfg.RUN.SAVE.PRESCALE)== 0)
                output['selected_events'][region] += processor.column_accumulator(events['event'][save_mask].astype(np.uint64))
            ...
            fill_mult('tight_ele_mult',electrons[events['is_tight_electron']])
            fill_mult('loose_muo_mult',muons)
            fill_mult('tight_muo_mult',muons[events['is_tight_muon']])
            ...
            ezfill('drelejet',    dr=events['dREleJet'][mask],      weight=rw[mask])
            ezfill('drmuonjet',    dr=events['dRMuonJet'][mask],      weight=rw[mask])
            ezfill('drphotonjet',    dr=events['dRPhotonJet'][mask],  weight=rw[mask])
            ...
            if region=='inclusive' or re.match('.*_v($|_.*)',region):
                ...
                if not events['is_data']:
                    ezfill('ak8_mass_response',   response=ak8[leadak8_index].mass[mask].max() / events['leadak8_gen_match_ak8_mass'][mask].max(),  weight=w_leadak8)
            ...
            # MET
            rw = region_weights.partial_weight(exclude=exclude)
            ezfill('dpfcalo',            dpfcalo=events["dPFCalo"][mask], weight=rw[mask])
            ezfill('dpftk',            dpfcalo=events["dPFTk"][mask], weight=rw[mask])
            ezfill('vec_b',            vec_b=events["vec_b"][mask], weight=rw[mask])
            ezfill('vec_dphi',            vec_dphi=events["vec_dphi"][mask], weight=rw[mask])

            ezfill('dphi_pf_tk',            dphi=events["dPhiTkPf"][mask], weight=rw[mask])
            ezfill('dphi_pf_calo',            dphi=events["dPhiCalPf"][mask], weight=rw[mask])
            ...
            ezfill('dphijm',             dphi=events["minDPhiJetMet"][mask],    weight=region_weights.partial_weight(exclude=exclude)[mask] )
            ezfill('dphijr',             dphi=events["minDPhiJetRecoil"][mask],    weight=region_weights.partial_weight(exclude=exclude)[mask] )
            ...
            if not events['is_data']:
                ezfill(
                        ...
                        recoil=recoil_pt[mask],
                        weight=(region_weights.partial_weight(exclude=exclude)*(1+events['weight_diboson_nlo_rel_unc']))[mask]
                        )
                ezfill(
            ...
            if cfg.RUN.PHOTON_ID_STUDY and (not events['is_data']) and ('cr_g' in region) and (events['year']!=2016):
                photon_id_sf_nom = evaluator['photon_id_tight_tnp'](np.abs(photons[events['is_tight_photon']].eta))
                photon_id_sf_err = evaluator['photon_id_tight_tnp_error'](np.abs(photons[events['is_tight_photon']].eta))
                ...
                photon_id_sf_err_extrap_err = evaluator['photon_id_tight_tnp_extrap_unc_slope'](np.abs(photons[events['is_tight_photon']].eta)) * (photons[events['is_tight_photon']].pt - 150)
            ...
            if cfg.RUN.ELE_ID_STUDY and (not events['is_data']) and ('cr_1e' in region or 'cr_2e' in region) and (events['year']!=2016):
                ...
                electron_is_tight_electron = events['is_tight_electron'][mask_electron_nogap]
            ...
            if re.match('.*no_veto.*', region) and not events['is_data']:
            ...
            if events['is_data'] and re.match("cr_g.*", region) and re.match("(SinglePhoton|EGamma).*", dataset):
                w_imp = photon_impurity_weights(photons[leadphoton_index].pt.max()[mask], events["year"])
                output['recoil'].fill(
            ...
            if '_1m_' in region or '_2m_' in region:
                ...
                ezfill('muon_mt',   mt=events['MT_mu'][mask],           weight=region_weights.partial_weight(exclude=exclude)[mask])
            ...
            if '_1e_' in region or '_2e_' in region:
                ...
                ezfill('electron_mt',   mt=events['MT_el'][mask],               weight=region_weights.partial_weight(exclude=exclude)[mask])
            ...
            if '_g_' in region:
                ...
                # w_drphoton_jet = weight_shape(events['dRPhotonJet'][mask], region_weights.partial_weight(exclude=exclude)[mask])

            # PV
            rweight = region_weights.partial_weight(exclude=exclude)
            rweight_nopu = region_weights.partial_weight(exclude=['pileup']+exclude)
            ezfill('npv', nvtx=events['PV_npvs'][mask], weight=rweight[mask])
            ezfill('npvgood', nvtx=events['PV_npvsGood'][mask], weight=rweight[mask])

            ezfill('npv_nopu', nvtx=events['PV_npvs'][mask], weight=rweight_nopu[mask])
            ezfill('npvgood_nopu', nvtx=events['PV_npvsGood'][mask], weight=rweight_nopu[mask])

            ezfill('rho_all', rho=events['fixedGridRhoFastjetAll'][mask], weight=rweight[mask])
            ezfill('rho_central', rho=events['fixedGridRhoFastjetCentral'][mask], weight=rweight[mask])
            ezfill('rho_all_nopu', rho=events['fixedGridRhoFastjetAll'][mask], weight=rweight_nopu[mask])
            ezfill('rho_central_nopu', rho=events['fixedGridRhoFastjetCentral'][mask], weight=rweight_nopu[mask])

            ezfill('npv_vs_recoil',     nvtx=events['PV_npvs'][mask],     recoil=recoil_pt[mask],   weight=rweight[mask])
            ezfill('npvgood_vs_recoil', nvtx=events['PV_npvsGood'][mask], recoil=recoil_pt[mask],   weight=rweight[mask])

            ezfill('npv_vs_recoil_nopu',     nvtx=events['PV_npvs'][mask],     recoil=recoil_pt[mask], weight=rweight_nopu[mask])
            ezfill('npvgood_vs_recoil_nopu', nvtx=events['PV_npvsGood'][mask], recoil=recoil_pt[mask], weight=rweight_nopu[mask])

            ezfill('rho_all_vs_recoil',          rho=events['fixedGridRhoFastjetAll'][mask],     recoil=recoil_pt[mask], weight=rweight[mask])
            ezfill('rho_central_vs_recoil',      rho=events['fixedGridRhoFastjetCentral'][mask], recoil=recoil_pt[mask], weight=rweight[mask])
            ezfill('rho_all_vs_recoil_nopu',     rho=events['fixedGridRhoFastjetAll'][mask],     recoil=recoil_pt[mask], weight=rweight_nopu[mask])
            ezfill('rho_central_vs_recoil_nopu', rho=events['fixedGridRhoFastjetCentral'][mask], recoil=recoil_pt[mask], weight=rweight_nopu[mask])
        return output
