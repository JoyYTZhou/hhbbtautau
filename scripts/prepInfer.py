...
if __name__ == "__main__":
    ...
    parser.add_argument('--file_name', type=str, default='prepared_data.csv', help='Name of the output csv file')
    args = parser.parse_args()
    df = load_dfs(args.data_dir, args.output_dir)
    prep_infer_input(df)
    df.to_csv(pjoin(args.output_dir, args.file_name), index=False)
    logging.info(f"Prepared data saved to {pjoin(args.output_dir, args.file_name)}")
