...
if [ "\$1" == "" ]; then
  export COFFEA_IMAGE=coffeateam/coffea-base:v2024.1.2
else
...
install_env() {
  ...
  .env/bin/python -m pip install dynaconf --no-cache-dir
  .env/bin/python -m pip install vector --upgrade --no-cache-dir
  .env/bin/python -m pip install dask --no-cache-dir
  .env/bin/python -m pip install dask-jobqueue --no-cache-dir
  .env/bin/python -m pip install dask-awkward --no-cache-dir
  .env/bin/python -m pip install dask-histogram --no-cache-dir
  .env/bin/python -m pip install htcondor --no-cache-dir
